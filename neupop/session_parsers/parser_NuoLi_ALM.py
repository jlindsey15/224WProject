...
class ParserNuoLiALM(SessionParser):
    '''
    SessionParser for Nuo Li's ALM perturbation data encoding,
    which comes in a .mat file.
        e.g. Data collected in 2014 - recording site is left ALM
    '''
    ...
    def parse_neural_unit_info(self):
        ...
        self.s.neural_unit_location = \
                np.asarray(['left_ALM']*self.s.neural_unit_type.size)
        return
    ...
    def parse_stim_present(self):
        ...
        self.s.stim_present = stim_type != 0
        return
    ...
    def parse_stim_period(self):
        ...
        '''
        stim_type = self.input['task_stimulation'][:,1]
        self.s.stim_period = np.zeros_like(stim_type)
        self.s.stim_period[stim_type == 1] = 2 #late sample
        self.s.stim_period[np.any([stim_type==2,stim_type==4,stim_type==5,stim_type==6], axis=0)] = 3 #early delay
        self.s.stim_period[stim_type == 3] = 4 #late delay
        return
    ...
    def parse_stim_period_num2str_dict(self):
        ...
        self.s.stim_period_num2str_dict = {0:'no stim', 2:'late sample', 3:'early delay', 4:'late delay'}
        return
    ...
    def parse_stim_site(self):
        ...
        '''
        stim_type = self.input['task_stimulation'][:,1]
        self.s.stim_site = np.zeros_like(stim_type)
        self.s.stim_site[np.any([stim_type==1,stim_type==2,stim_type==3,stim_type==4], axis=0)] = 1 #left ALM silencing
        self.s.stim_site[stim_type == 5] = 2 #right ALM silencing
        self.s.stim_site[stim_type == 6] = 3 #bi ALM silencing
        return
